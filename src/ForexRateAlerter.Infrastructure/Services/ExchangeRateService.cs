...
namespace ForexRateAlerter.Infrastructure.Services
{
    public class ExchangeRateService : IExchangeRateService
    {
        ...
        {
            // Use database-side grouping to fetch only the latest rate for each pair
            var latestRates = await _context
        ...
        public async Task<IEnumerable<EnrichedExchangeRateDto>> GetEnrichedRatesAsync()
        {
            ...
            var allRatesInWindow = await _context
                .ExchangeRates.Where(r => r.Timestamp >= fromDate)
                .ToListAsync();
            ...
            return latestRates.Select(rate =>
            {
                var s = stats.FirstOrDefault(x =>
                    x.BaseCurrency == rate.BaseCurrency && x.TargetCurrency == rate.TargetCurrency
                );
                return new EnrichedExchangeRateDto
                {
                    ...
                    Change24h =
                        (s != null && s.Open != 0) ? ((rate.Rate - s.Open) / s.Open) * 100 : 0,
                };
            });
        }
        ...
        public async Task<bool> FetchAndStoreLatestRatesAsync()
        {
            try
            {
                ...
                var existingRates = (await GetLatestRatesAsync()).ToDictionary(
                ...
                foreach (var baseCurrency in _supportedCurrencies)
                {
                    ...
                    {
                        _logger.LogError(
                            $"Failed to fetch rates for {baseCurrency}: {response.StatusCode}"
                        );
                        continue;
                    }
                ...
                if (anyChanges)
                {
                    ...
                    _logger.LogInformation(
                        $"Successfully fetched and stored updated exchange rates at {timestamp}"
                    );
                }
                else
                {
        ...
        public async Task<IEnumerable<OHLCData>> GetOHLCDataAsync(
            string baseCurrency,
            string targetCurrency,
            string timeframe = "1h",
            int limit = 100
        ...
        )
        {
            return intervalType switch
            {
                "m" => rates.GroupBy(r => new DateTime(
                    ...
                    r.Timestamp.Day,
                    r.Timestamp.Hour,
                    (r.Timestamp.Minute / interval) * interval,
                    0
                )),
                "h" => rates.GroupBy(r => new DateTime(
                    ...
                    r.Timestamp.Month,
                    r.Timestamp.Day,
                    (r.Timestamp.Hour / interval) * interval,
                    0,
                    0
                )),
        ...
        [System.Obsolete(
            "This method is deprecated - use IExchangeRateHistoryService.GetTopMoversAsync instead"
        )]
